...
def plot_at_k(at_step_k, k_steps, extra=""): 
    ...
    os.makedirs("outputs_q2_spiral/k_step", exist_ok=True) 
    ...
    for idx, k in enumerate(k_steps): 
        ...
        ax.grid(alpha=0.2) 
    plt.tight_layout() 
    plt.savefig("outputs_q2_spiral/k_step/" + extra + "grid_plot.png", dpi=200) 
    plt.close()
...
def grid_2d_vector(model, device, t_vals, lim=2.5):
    if len(t_vals) != 6:
        raise ValueError("Must be 6 time steps")
    os.makedirs("outputs_q2_spiral/vector_grid", exist_ok=True) 
    ...
    plt.tight_layout() 
    plt.savefig("outputs_q2_spiral/vector_grid/grid_plot.png", dpi=200) 
    plt.close()

'''
True vs denoised
'''
def comparison_plot(x_true: np.ndarray, x_denoise: np.ndarray = None, x_approx :np.ndarray = None, save="outputs_q2_spiral/k_step/"):
    approx = ""
